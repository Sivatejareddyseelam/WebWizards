...
class Customer(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    customer_first_name: so.Mapped[str] = so.mapped_column(sa.String(64), index=True)
    customer_last_name: so.Mapped[str] = so.mapped_column(sa.String(64), index=True)
    customer_email: so.Mapped[str] = so.mapped_column(sa.String(120), index=True,
                                             unique=True)
    customer_phone: so.Mapped[str] = so.mapped_column(sa.String(120), index=True,
                                             unique=True)
    ...
    api_call_limit: so.Mapped[Optional[int]] = so.mapped_column(sa.Integer)
